...
export default function TicketDetailsScreen() {
  ...
  if (!ticket) {
    return (
      <AppShell>
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }} >
          <Text style={{ color: Colors.text.secondary }}>Ticket not found.</Text>
        </View>
      </AppShell>
    );
  }
    ...
    <AppShell>
      <View style={{ flex: 1, paddingHorizontal: 16, paddingTop: 16 }}>
        ...
        >
          ...
          <Text style={{ color: Colors.text.secondary, fontSize: 18, fontWeight: '700', marginBottom: 8 }} >
            {ticket.subject}
          </Text>
          ...
          {/* comments line */}
          <View style={{ height: 1, backgroundColor: Colors.appBackground, marginVertical: 24, width: '100%'}} />
          ...
          {ticket.comments.length === 0 ? (
            <Text style={{ color: Colors.icon.default, fontStyle: 'italic' }}>No comments yet.</Text>
          ) : (
            <View style={{ gap: 16 }}>
              {ticket.comments.map((comment) => {
                ...
                return (
                  <View key={comment.id} style={{ backgroundColor: Colors.border, borderRadius: 12, padding: 12 }} >
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                      ...
                      <Text style={{ color: Colors.icon.default, fontSize: 12 }}>
